...
namespace SimilarSentence
{
	...
	public class SimilarGenerator
	{
		...
		const int ContextSize=6;//Local disambiguation within the context size 
		private string[][][][] _dictInfo ;//[words][relations][tokens]

	    private readonly Tokeniser _tokenize = new Tokeniser();

	    private readonly ArrayList list = new ArrayList();
	    readonly MyWordInfo[] _myPos;
		int _numItems=0;
		int _numWord;
		...
		{			
			...
			if (_alterWord[wordIndex] != null )
			{
				...
				if (!ReadWordSenseInfo (wordIndex))  return -1;
		...
		{
			float temperature=1;
			const float annealSchedule = 0.9F;
			const int maxTrial = 100;
			var maxSucceed=10; // 1/10 of N_TRIAL
			...
			{								
				...
				{
					MyWordInfo[] trial;
					int newCost=GetNeighbour (current,out trial);

					if ( newCost != -1)
					{
					    float delta=newCost - current_cost;
			...
			if (_alterWord[index] != null && _alterWord[index].Length > 0)
			{
				for (int j = 0; j < _alterWord[index].Length; j++)
					if (dx[index][j] == 0)
					{
						...
						int delta=0;
						if (ReadWordSenseInfo (index)) delta=ComputeScore(index);
						_overallScore +=delta;
		...
		private bool ReadWordSenseInfo(int wordIndex)
		{
		...
		private static int ScoringSensePair(string[][] sense1, string[][] sense2)
		{
			int score=0;
