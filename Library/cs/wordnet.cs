		...
		void findtheinfo()
		{
			...
			switch (sch.ptp.Mnemonic)
			{
				...
				case "FREQ":
					...
					{
						countSenses.Add(idx.SynsetOffsets.Length);
						buf += "Sense " + countSenses.Count + ": " +
							idx.SynsetOffsets.Length;
					}
					break;
				case "WNGREP":
				...
				default:
					...
					while ((idx = ixs.next()) != null)
					{
						...
						for (int sense = 0; sense < idx.SynsetOffsets.Length; sense++)
							if (whichsense == ALLSENSES || whichsense == sense + 1)
							{
								...
								for (int j = 0; j < senses.Count; j++)
								{
									...
									if (ss.hereiam == idx.SynsetOffsets[sense])
										goto skipit;
								}
	...
	*/
	 ...
	 */
		...
		RelList findVerbGroups(Index idx, RelList rellist)
		{
			...
			{
				SynSet synset = new SynSet(idx.SynsetOffsets[i], pos, idx.Wd, this, i);
				...
				for (j = 0; j < synset.ptrs.Length; j++)
				{
					...
					if (p.ptp.Mnemonic == "VERBGROUP")
						...
						for (k = 0; k < idx.SynsetOffsets.Length; k++)
							if (p.off == idx.SynsetOffsets[k])
							{
		...
		void doRelList(Index idx, RelList rellist)
		{
			...
			for (RelList rel = rellist; rel != null; rel = rel.next)
			{
				...
				for (i = 0; i < idx.SynsetOffsets.Length; i++)
					...
					{
						flag = true;
						synptr = new SynSet(idx.SynsetOffsets[i], pos, "", this, i);
						synptr.strsns(i + 1);
